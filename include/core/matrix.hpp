...
public:
  ...
  Matrix roi(const Rect &region) const {
    ...
    // 复制ROI数据
    const uint8_t *src_ptr = data_.data() +
                             static_cast<size_t>(region.y) * step_ +
  ...
  bool contains(const Rect &region) const {
    return region.x >= 0 && region.y >= 0 && region.x + region.width <= cols_ &&
  ...
  void printInfo(const std::string &name = "Matrix") const {
    std::cout << name << ": " << rows_ << "x" << cols_
              << ", type=" << dataTypeName(dtype_) << ", step=" << step_
              << " bytes" << std::endl;
  }

  template <typename T> void print(int maxRows = 10, int maxCols = 10) const {
...
private:
  ...
  template <typename T> void fillValue(const Scalar &value) {
    T *d = ptr<T>();
    size_t total_elements = total();
