...
public:
  ...
  template <typename T> T *ptr(int row = 0) { return mat_.ptr<T>(row); }

  template <typename T> const T *ptr(int row = 0) const {
    return mat_.ptr<T>(row);
  }

  // 像素访问（单通道）
  template <typename T = uint8_t> T at(int y, int x, int channel = 0) const {
    return mat_.at<T>(y, x * channels_ + channel);
  }

  template <typename T = uint8_t> T &at(int y, int x, int channel = 0) {
    return mat_.at<T>(y, x * channels_ + channel);
  }

  // 像素访问（多通道）
  template <typename T = uint8_t> Vec<T, 3> at3(int y, int x) const {
    if (channels_ < 3)
      throw std::runtime_error("Image has less than 3 channels");
  ...
  template <typename T = uint8_t> Vec<T, 4> at4(int y, int x) const {
    if (channels_ < 4)
      throw std::runtime_error("Image has less than 4 channels");
  ...
  Image roi(const Rect &region) const {
    // 调整ROI到实际像素坐标
    Rect pixel_region = Rect(region.x * channels_, region.y,
                             region.width * channels_, region.height);
  ...
  Image pixel_roi(const Rect &pixel_region) const {
    Rect mat_region = Rect(pixel_region.x * channels_, pixel_region.y,
                           pixel_region.width * channels_, pixel_region.height);
  ...
  // 克隆
  Image clone() const { return Image(mat_.clone(), channels_); }

  // 创建图像
  void create(int width, int height, int channels = 1,
              DataType dtype = DataType::UINT8) {
    channels_ = channels;
    mat_.create(height, width * channels, dtype);
  }

  void create(const Size &size, int channels = 1,
              DataType dtype = DataType::UINT8) {
  ...
  // 填充操作
  void setTo(Scalar value) { mat_.setTo(value); }
    ...
    for (const auto &ch : channels) {
      if (ch.height() != height || ch.width() != width || ch.type() != dtype) {
        throw std::invalid_argument(
            "All channels must have same size and type");
      }
    }
  ...
  void printInfo(const std::string &name = "Image") const {
    std::cout << name << ": " << width() << "x" << height() << "x" << channels()
  ...
  public:
    PixelIterator(Image *img, int x = 0, int y = 0)
        : image_(img), x_(x), y_(y), width_(img->width()),
          height_(img->height()) {}
    ...
    template <typename T = uint8_t> Vec<T, 3> pixel3() const {
      return image_->template at3<T>(y_, x_);
    }

    template <typename T = uint8_t> T pixel(int channel) const {
...
inline Image flipHorizontal(const Image &src) {
  ...
  for (int y = 0; y < src.height(); ++y) {
    for (int x = 0; x < src.width(); ++x) {
      for (int c = 0; c < src.channels(); ++c) {
        result.at<uint8_t>(y, src.width() - 1 - x, c) =
            src.at<uint8_t>(y, x, c);
      }
    }
  }
...
inline Image flipVertical(const Image &src) {
  ...
  for (int y = 0; y < src.height(); ++y) {
    for (int x = 0; x < src.width(); ++x) {
      for (int c = 0; c < src.channels(); ++c) {
        result.at<uint8_t>(src.height() - 1 - y, x, c) =
            src.at<uint8_t>(y, x, c);
      }
    }
  }
...
inline Image rotate90(const Image &src, bool clockwise = true) {
  ...
  if (clockwise) {
    for (int y = 0; y < src.height(); ++y) {
      for (int x = 0; x < src.width(); ++x) {
        for (int c = 0; c < src.channels(); ++c) {
          result.at<uint8_t>(x, src.height() - 1 - y, c) =
              src.at<uint8_t>(y, x, c);
        }
      }
    }
    ...
    for (int y = 0; y < src.height(); ++y) {
      for (int x = 0; x < src.width(); ++x) {
        for (int c = 0; c < src.channels(); ++c) {
          result.at<uint8_t>(src.width() - 1 - x, y, c) =
              src.at<uint8_t>(y, x, c);
        }
      }
    }
