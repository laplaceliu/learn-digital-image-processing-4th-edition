...
public:
  // 从文件加载图像
  static std::shared_ptr<Image> load_from_file(const std::string &filename,
    ...
    std::copy(data, data + width * height * actual_channels,
              image->ptr<uint8_t>());
  ...
  // 从内存加载图像
  static std::shared_ptr<Image> load_from_memory(const unsigned char *buffer,
                                                 int len,
                                                 int desired_channels = 0) {
    int width, height, channels;
    ...
    if (!data) {
      std::cerr << "Error: Failed to load image from memory: "
                << stbi_failure_reason() << std::endl;
      return nullptr;
    }
  ...
  // 获取图像信息（不加载完整图像数据）
  static bool get_image_info(const std::string &filename, int &width,
  ...
  static bool save_as_ppm(const Image &image, const std::string &filename) {
    ...
    if (image.channels() == 1) {
      ...
      std::cerr << "Error: Only 1 or 3 channel images supported for PPM saving"
                << std::endl;
      return false;
    }
  ...
  // 保存图像为二进制PPM格式
  static bool save_as_ppm_binary(const Image &image,
    ...
    if (image.channels() == 1) {
      ...
      for (int y = 0; y < image.height(); ++y) {
        file.write(reinterpret_cast<const char *>(image.ptr<uint8_t>(y)),
                   image.width());
      }
    } else if (image.channels() == 3) {
      // PPM binary
      ...
      std::cerr
          << "Error: Only 1 or 3 channel images supported for PPM binary saving"
          << std::endl;
      return false;
    }
  ...
  static bool save_as_pgm(const Image &image, const std::string &filename) {
    if (image.channels() != 1) {
      std::cerr << "Error: Only single channel images can be saved as PGM"
                << std::endl;
      return false;
    }
  ...
  // 保存图像为PGM二进制格式
  static bool save_as_pgm_binary(const Image &image,
                                 const std::string &filename) {
    if (image.channels() != 1) {
      std::cerr
          << "Error: Only single channel images can be saved as PGM binary"
          << std::endl;
      return false;
    }
