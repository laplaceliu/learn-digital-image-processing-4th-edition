...
public:
    ...
    Vec(const Vec<T2, N>& other) {
        for(int i = 0; i < N; ++i) data[i] = static_cast<T>(other[i]);
    }
    ...
    template<typename T2>
    Vec& operator=(const Vec<T2, N>& other) {
        for(int i = 0; i < N; ++i) data[i] = static_cast<T>(other[i]);
        return *this;
    }
    ...
    T norm() const {
        if constexpr (P == 1) {
            T result = T(0);
            for(int i = 0; i < N; ++i) result += std::abs(data[i]);
            return result;
        } else if constexpr (P == 2) {
            return length();
        } else if constexpr (P == 0) {  // L-infinity
            T result = T(0);
            for(int i = 0; i < N; ++i) result = std::max(result, std::abs(data[i]));
            return result;
        }
    }
    ...
    T min() const {
        T result = data[0];
        for(int i = 1; i < N; ++i) result = std::min(result, data[i]);
        return result;
    }
    
    T max() const {
        T result = data[0];
        for(int i = 1; i < N; ++i) result = std::max(result, data[i]);
        return result;
    }
    ...
    // 获取子向量
    template<int M>
    Vec<T, M> subvec(int start = 0) const {
        Vec<T, M> result;
    ...
    static Vec ones() {
        Vec result;
        for(int i = 0; i < N; ++i) result[i] = T(1);
        return result;
    }
...
public:
    ...
    Scalar(double v0) { val[0] = v0; val[1] = val[2] = val[3] = 0; }
    Scalar(double v0, double v1) { val[0] = v0; val[1] = v1; val[2] = val[3] = 0; }
    Scalar(double v0, double v1, double v2) { val[0] = v0; val[1] = v1; val[2] = v2; val[3] = 0; }
    Scalar(double v0, double v1, double v2, double v3) { 
        val[0] = v0; val[1] = v1; val[2] = v2; val[3] = v3; 
    }
    
    template<typename T, int N>
    Scalar(const Vec<T, N>& vec) {
        for(int i = 0; i < 4; ++i)
    ...
    bool operator==(const Scalar& other) const {
        for(int i = 0; i < 4; ++i) {
            if(std::abs(val[i] - other[i]) > 1e-10) return false;
        }
        return true;
    }
    ...
    Scalar operator+(const Scalar& other) const {
        Scalar result;
        for(int i = 0; i < 4; ++i) result[i] = val[i] + other[i];
        return result;
    }
    ...
    Scalar operator*(double scale) const {
        Scalar result;
        for(int i = 0; i < 4; ++i) result[i] = val[i] * scale;
        return result;
    }
