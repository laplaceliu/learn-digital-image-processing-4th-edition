...
int main(int argc, char *argv[]) {
  ...
  spdlog::info("Image Downsampling Test");

  std::string filename = argc > 1 ? argv[1] : "test.png";
  int factor = argc > 2 ? std::atoi(argv[2]) : 2;

  spdlog::info("Loading image: {}, factor: {}x", filename, factor);
  ...
  try {
    auto result = algorithms::downsample(*img, factor);
    spdlog::info("Downsampled image: {}x{}x{}", result.width(), result.height(),
                 result.channels());

    std::string output = "downsampled_" + std::to_string(factor) + "x.ppm";
