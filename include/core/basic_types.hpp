...
struct Point {
    ...
    template<typename T2>
    bool operator<(const Point<T2>& other) const {
...
struct Rect {
    ...
    Rect(const Point2i& pt, const Size& sz) : x(pt.x), y(pt.y), width(sz.width), height(sz.height) {}
    ...
    Rect operator&(const Rect& other) const {
        ...
        if (x2 <= x1 || y2 <= y1) return Rect();  // 无交集
